...
export function MediaEditor({
  ...
  function handleRemove(id: string) {
    ...
    startTransition(async () => {
      ...
      toast.success(t("editorForms.media.toasts.deleted"));
      router.refresh();
    });
  }
  ...
  function handleArchive(id: string) {
    ...
    startTransition(async () => {
      ...
      toast.success(t("editorForms.media.toasts.archived"));
      router.refresh();
    });
  }
  ...
  function handleRestore(id: string) {
    ...
    startTransition(async () => {
      ...
      toast.success(t("editorForms.media.toasts.restored"));
      router.refresh();
    });
  }
  ...
  function handleReplace(entry: Entry, file: File) {
    ...
    startTransition(async () => {
      try {
        ...
        toast.success(t("editorForms.media.toasts.replaced"));
        router.refresh();
      } catch (err) {
        toast.error(err instanceof Error ? err.message : t("editorForms.media.toasts.replaceFailed"));
      } finally {
        setReplacingId(null);
      }
    });
    ...
    <div className="rounded-2xl border border-mist p-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-heading text-base font-bold text-navy dark:text-white">
              {t("editorForms.media.heading")}
            </h2>
          </div>
          <p className="mt-1 text-xs text-slate">
            {t("editorForms.media.description")}
          </p>
        </div>
        {archivedEntries.length > 0 && (
          ...
          >
            ...
            {t("editorForms.media.archivedButton")} ({archivedEntries.length})
          </button>
        )}
      </div>
      ...
      {!locked && (
        <div className="mt-4">
          <MultiFileUpload
            ...
            label={t("editorForms.media.uploadLabel")}
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          <div className="flex h-9 min-w-[160px] flex-1 items-center gap-2 rounded-lg border border-mist px-3">
            ...
            <input
              ...
              placeholder={t("editorForms.media.searchPlaceholder")}
          ...
          >
            <option value="all">{t("editorForms.media.typeFilter.all")}</option>
            <option value="photo">{t("editorForms.media.typeFilter.photo")}</option>
            <option value="video">{t("editorForms.media.typeFilter.video")}</option>
            <option value="audio">{t("editorForms.media.typeFilter.audio")}</option>
            <option value="document">{t("editorForms.media.typeFilter.document")}</option>
          </select>
          <select
            value={sortBy}
            ...
            className="h-9 rounded-lg border border-mist bg-white px-2 text-xs text-navy focus:border-teal focus:outline-none dark:bg-offwhite dark:text-white"
          >
            <option value="newest">{t("editorForms.media.sortBy.newest")}</option>
            <option value="oldest">{t("editorForms.media.sortBy.oldest")}</option>
            <option value="type">{t("editorForms.media.sortBy.type")}</option>
          </select>
          <div className="flex rounded-lg border border-mist p-0.5">
            <button
              type="button"
              onClick={() => setViewMode("grid")}
              aria-label={t("editorForms.media.gridViewLabel")}
              className={cn(
            ...
            <button
              ...
              aria-label={t("editorForms.media.listViewLabel")}
              className={cn(
                "flex h-7 w-7 items-center justify-center rounded-md transition-colors",
      ...
      {filtered.length === 0 ? (
        <p className="py-10 text-center text-sm text-slate">
          {baseList.length === 0
            ? showArchived
              ? t("editorForms.media.emptyStateArchived")
              : t("editorForms.media.emptyStateNone")
            : t("editorForms.media.emptyStateSearch")}
        </p>
                  ...
                  <div className="ml-auto flex items-center gap-2">
                    {showArchived ? (
                      ...
                      >
                        ...
                        {t("editorForms.media.restoreLabel")}
                      </button>
                    ) : (
                      <>
                        <button
                        ...
                        >
                          ...
                          {t("editorForms.media.detailsButton")}
                        </button>
                        <button
                          type="button"
                          onClick={() => handleArchive(entry.id)}
                          disabled={pending}
                          title={t("editorForms.media.archiveLabel")}
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-semibold text-navy dark:text-white">
                    {entry.title ?? entry.file_name ?? t("editorForms.media.untitled")}
                  </p>
                ...
                {!locked && (
                  <div className="flex shrink-0 items-center gap-1.5">
                    {showArchived ? (
                      <ActionIconButton
                        icon={ArchiveRestore}
                        label={t("editorForms.media.restoreLabel")}
                        tone="teal"
                        onClick={() => handleRestore(entry.id)}
                        disabled={pending}
                      />
                      ...
                      <>
                        <ActionIconButton
                          icon={Pencil}
                          label={t("editorForms.media.editDetailsLabel")}
                          tone="teal"
                          onClick={() => setEditingId(entry.id)}
                          disabled={pending}
                        />
                        <ActionIconButton
                          icon={Archive}
                          label={t("editorForms.media.archiveLabel")}
                          tone="gold"
                          onClick={() => handleArchive(entry.id)}
                          disabled={pending}
                        />
...
function MediaMetaForm({
  ...
  const errorMessage = state.errorCode
    ? t(`editorForms.media.errors.${state.errorCode}`)
    : state.error;
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <p className="text-xs font-semibold uppercase tracking-wide text-teal">{t("editorForms.media.detailsHeading")}</p>
      ...
      <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
        <Field label={t("editorForms.media.fields.title")} name="title" defaultValue={entry.title ?? ""} />
        <Field label={t("editorForms.media.fields.category")} name="category" defaultValue={entry.category ?? ""} />
        <Field label={t("editorForms.media.fields.date")} name="event_date" type="date" defaultValue={entry.event_date ?? ""} />
        <Field label={t("editorForms.media.fields.location")} name="location" defaultValue={entry.location ?? ""} placeholder={t("editorForms.media.fields.locationPlaceholder")} />
      </div>

      <div className="mt-3">
        <label className="text-xs font-semibold uppercase tracking-wide text-slate">{t("editorForms.media.fields.description")}</label>
        <textarea
          name="caption"
          rows={2}
      ...
      <label className="mt-3 flex items-center gap-2 text-xs font-medium text-slate">
        ...
        />
        {t("editorForms.media.featureOnProfile")}
      </label>
      ...
      <div className="mt-4 flex justify-end gap-2">
        ...
        >
          ...
          {t("editorForms.media.saveDetailsButton")}
        </button>
      </div>
    </form>
  );
